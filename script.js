...
(function () {
    ...
    /* ========== ADMIN DATA LOADER ========== */
    var ADMIN_STORAGE_KEY = 'mi-pos-admin-data';
    ...
    function applyAdminData() {
        ...
        if (data.pricing) {
            ...
            prefixes.forEach(function (prefix) {
                priceKeys.forEach(function (key) {
                    var info = data.pricing[key];
                    if (!info) return;

                    var card = document.querySelector('[data-plan="' + prefix + '-' + key + '"]');
                    if (!card) return;

                    var accountsEl = card.querySelector('.plan-accounts');
                    if (accountsEl && info.accounts) accountsEl.textContent = info.accounts;
                    ...
                    var priceField = prefix === 'ot' ? info.otPrice : info.moPrice;
                    if (priceEl && priceField) priceEl.textContent = priceField;

                    var installEl = card.querySelector('.plan-install');
                    var installField = prefix === 'ot' ? info.otInstall : info.moInstall;
                    if (installEl && installField) installEl.textContent = '+ ' + installField;
                });
            });
        }
        ...
        if (data.variants) {
            ...
            varKeys.forEach(function (key) {
                var info = data.variants[key];
                if (!info) return;
                var card = document.querySelector('[data-variant="' + key + '"]');
                if (!card) return;

                var iconEl = card.querySelector('.variant-icon');
                if (iconEl && info.icon) iconEl.textContent = info.icon;

                var nameEl = card.querySelector('.variant-name');
                if (nameEl && info.name) nameEl.textContent = info.name;

                var descEl = card.querySelector('.variant-desc');
                if (descEl && info.desc) descEl.textContent = info.desc;

                var badgeEl = card.querySelector('.variant-badge');
                if (badgeEl && info.badge) {
                    badgeEl.textContent = info.badge;
                    badgeEl.className = 'variant-badge';
                    if (info.color) badgeEl.classList.add('variant-badge-' + info.color);
                }
            });
        }
        ...
        if (data.hardware) {
            ...
            hwKeys.forEach(function (key) {
                var info = data.hardware[key];
                if (!info) return;
                var card = document.querySelector('[data-hardware="' + key + '"]');
                if (!card) return;

                var iconEl = card.querySelector('.hardware-icon');
                if (iconEl && info.icon) iconEl.textContent = info.icon;

                var nameEl = card.querySelector('.hardware-name');
                if (nameEl && info.name) nameEl.textContent = info.name;

                var descEl = card.querySelector('.hardware-desc');
                if (descEl && info.desc) descEl.textContent = info.desc;

                var badgeEl = card.querySelector('.hardware-badge');
                if (badgeEl && info.badge) {
                    badgeEl.textContent = info.badge;
                    badgeEl.className = 'hardware-badge';
                    if (info.color) badgeEl.classList.add('hardware-badge-' + info.color);
                }
            });
        }
        ...
        if (data.whatsapp) {
            ...
            var msg = '?text=I%20want%20to%20buy%20MI%20Smart%20POS';
            var waLinks = document.querySelectorAll('.wa-link');
            waLinks.forEach(function (link) {
                link.href = 'https://wa.me/' + wa.primary + msg;
            });
            var primaryLink = document.querySelector('.wa-link-primary');
            if (primaryLink) primaryLink.href = 'https://wa.me/' + wa.primary + msg;
            var secondaryLink = document.querySelector('.wa-link-secondary');
            if (secondaryLink) secondaryLink.href = 'https://wa.me/' + wa.secondary + msg;

            /* Also update pricing card WA links with plan-specific messages */
            document.querySelectorAll('.price-card[data-plan]').forEach(function (card) {
                ...
                var name = planName ? planName.textContent.trim() : plan;
                var typeLabel = plan.startsWith('ot') ? 'One%20Time' : 'Monthly';
                var link = card.querySelector('.wa-link');
                if (link) {
                    link.href = 'https://wa.me/' + wa.primary + '?text=I%20want%20to%20buy%20MI%20Smart%20POS%20' + typeLabel + '%20-%20' + encodeURIComponent(name);
                }
            });
        }
    }
